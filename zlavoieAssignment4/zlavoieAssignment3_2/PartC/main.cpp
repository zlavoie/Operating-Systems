...
pthread_t filters[100]; //Max number of threads for thread array
...
void *SplitCylWork(void* startCell); //Helper thread in Iterations that divides the cylinder for different threads to recalc values
...
in >> Delta; //Set Delta
    in.clear();
    in << argv[9];
in >> NumThreads; //Set MAX iterations
    in.clear();
cout<<"\nHeight: "<<H<<" Circ: "<<C<<endl;
cout<<"tinit: "<<tinit<<" C0,C1,C2: "<<C0<<", "<<C1<<", "<<C2<<endl;
cout<<"Delta: "<<Delta<<" Max: "<<MAX<<" Number of Threads: "<<NumThreads<<"\n"<<endl;
...
void Iterations(){
  ...
  for(int i=0;i<MAX;i++){ //Max iterations of the simulation
        ...
        InitializePrev();
	if((H*C)%NumThreads==0){
        cells = (H*C)/NumThreads;
	}else{NumThreads++;}
	
        startCell = 0;
        //Max cyl: 100 x 100
        //Max # Threads: 100
        //Split sections by rows: H/MAX = # rows to look at in each section
        //How many threads are working on each section at once   
	for(int t=0; t<NumThreads;t++){
	  pthread_create(&filters[t],NULL,SplitCylWork,(void *)(intptr_t)startCell);
	  startCell=startCell+cells;
	}
...
void *SplitCylWork(void* startCell){
	   int start = ((intptr_t)startCell);
	//cout<<"START: "<<start<<endl;
    //Running it for MAX times
    bool fixed;
          ...
          for(int h=startRow;h<endRow;h++){
		   ...
		   //	FLOPS++;
                //have to consider no D0,N0,nor D1 for weighted average
                   ...
                   newTemp = round(newTemp + neighbor + diag);
		   //	FLOPS++;
                   cyl[h][c] = newTemp;
